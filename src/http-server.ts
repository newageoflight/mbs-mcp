...
function loadMBSData(): void {
  ...
  console.log(`Loaded ${mbsItems.length} MBS items from ${loadedPath}`);
}
...
async function main() {
  ...
  app.get("/health", (_req: Request, res: Response) => {
    res.status(200).json({ status: "healthy", items: mbsItems.length });
  });
  ...
  app.listen(PORT, () => {
    console.log(`Medicare Australia MCP server running on http://localhost:${PORT}`);
